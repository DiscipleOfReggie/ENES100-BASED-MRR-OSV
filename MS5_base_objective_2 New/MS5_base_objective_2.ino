...
int power = 255;
const int thresh = .5;
const double obstacleDistance = 23.5;
void setup() {
  Serial.begin(9600);
  osv.init();
  cur_phase = phase_0;
  ...
  switch (cur_phase) {
    ...
    */
    case phase_0:
      ...
      while (enes.location.theta <= 3.14 - thresh) {
        enes.updateLocation();
        osv.turnLeft(power);
        delay(500);
      }

      while (enes.location.x <= 1.375 + thresh) {
    ...
    */
    case phase_1:
      ...
      while (enes.location.x <= 2.7 + thresh) {
        ...
        while (osv.obstacle(obstacleDistance)) {
          osv.turnRight(power);
